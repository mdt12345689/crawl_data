...
def split_data(padded, labels):
    ...
    X_train,X_,y_train,y_=train_test_split(padded, labels,random_state=2018, train_size=0.5, stratify=labels)
...
def convert(input_string):
    ...
    if match:
        # Nếu tìm thấy số, chuyển đổi nó thành integer và trả về
        result = int(match.group())
        return result
    else:
        # Nếu không tìm thấy số, có thể xử lý theo cách khác hoặc trả về giá trị mặc định
        return None
...
def result(sentence):
    ...
    result_string = str(preds[0])
    return result_string
#run(50)
with open('text.txt', 'r', encoding='utf-8') as file:
    lines_text = file.readlines()

# Khởi tạo mảng để lưu trữ các link
links = []

# Mở tệp link.txt và đọc từng dòng
with open('link.txt', 'r', encoding='utf-8') as link_file:
    # Duyệt qua từng dòng và thêm vào mảng links
    for line in link_file:
        links.append(line.strip())
#Duyêt nội dung content với mỗi link
index = 0
num1 = 0
num0 = 0
for line_text in lines_text:
    if index < len(links):
        ...
        sentence = line_text.strip()
        if not sentence:
            if num1*5 > num0:
                print (f"{links[index]}: 1")
            else:
                print (f"{links[index]}: 0")
            index += 1
            num0 = 0
            num1 = 0
        else:
            if (convert(result(sentence)) == 1):
                num1 += 1
            else:
                num0 += 1

        #print (sentence)
        #print (convert(result(sentence)))
